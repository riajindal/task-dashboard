...
function Sidebar() {
    ...
    const Menus = [
        { title: "Add Project", src: <FolderPlusIcon className='h-6 w-6' /> },
        { title: "Add Task", src: <CheckCircleIcon className='h-6 w-6' /> },
    ...
    <ul className='pt-6'>
      {Menus.map((menu, index) => {
        return <li key={index} className={`text-gray-300 text-sm flex items-center cursor-pointer gap-x-4 p-2 hover:bg-slate-400 rounded-md ${menu.gap ? "mt-7" : "mt-2"}`} >
          {menu.src}
